document.addEventListener("DOMContentLoaded", function () {
    ...
    const minY = settings.querySelector("#minAge");
    const maxY = settings.querySelector("#maxAge");
    const avgY = settings.querySelector("#avgAge");
    ...
    avgY.addEventListener("change", clearErr);
    ...
    buildBtn.addEventListener("click", function () {
        ...
        const series = [];
        if (minY.checked) series.push("minAge");
        if (maxY.checked) series.push("maxAge");
        if (avgY.checked) series.push("avgAge");
        ...
        clearGraph();
        drawGraph(places, series, keyX.value, typeGraph.value);
    });
});
